...
class Certs(object):
    ...
    def dojotLogin(self, dojot, username, passwd, httpsVerify):
        try:
            r = requests.post(
                            dojot + conf.authpath, verify=httpsVerify,
                            json={
                                "username": username,
                                "passwd": passwd
                            })
            ...
            self._logger.info(
                "Could not connect to Auth service at " + dojot + conf.authpath
                )
            exit(-1)
    ...
    def generateKeys(self, devname, length, overwrite):
        ...
        if not os.path.isfile(filename) or overwrite:
            ...
            self._logger.info(
                "Key pair already exists at " + filename + ". Skiping."
                )
    ...
    def generateCSR(self, devname, overwrite, dns, ip):
        ...
        if not os.path.isfile(filename) or overwrite:
            ...
            self._logger.info(
                "CSR file already exists at " + filename + ". Skiping."
                )
    ...
    def askCertSign(self, ejbcaHost, devname, overwrite):
        ...
        if not os.path.isfile(filename) or overwrite:
            ...
            self._logger.info(
                devname +
                " certificate signed. Avaliable at " +
                filename
                )
        else:
            self._logger.info(
                "Certificate file already exists at " +
                filename +
                ". Skiping."
                )
    ...
    def generateCerts(self):
        ...
        self.retrieveCAChain(
            url,
            self.userConf.caName,
